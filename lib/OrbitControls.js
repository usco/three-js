...
THREE.OrbitControls = function ( object, domElement, upVector ) {
    ...
    function onMouseUp( event ) {
        if ( scope.enabled === false ) return;
        if ( scope.userRotate === false ) return;
        scope.domElement.removeEventListener( 'pointermove', onMouseMove, false );
        scope.domElement.removeEventListener( 'pointerup', onMouseUp, false );
        state = STATE.NONE;
    }

    function onMouseWheel( event ) {
        //console.log("mousewheeling", event);
        if ( scope.enabled === false ) return;
    ...
    function onPointerMove( event )
    {
      ...
      //is primary
        ...
        if ( state === STATE.ROTATE ) {
            ...
            var pointerData = scope.pointers[event.pointerId];
            if(event.pointerId && pointerData)
            {
              ...
              scope.pointers[event.pointerId] = newPointerData;
            }
        }
    }
