...
function SelectionHelper(options) {
	...
	SelectionHelper.prototype.pick = function(x,y,isOrtho){
		...
		if( !isOrtho)
		{
		  ...
		  intersects = raycaster.intersectObjects(this.hiearchyRoot, true);
		  				//var e = Date.now();
		  //onsole.log( "Time to pick (internal)" + (e-s) + " ms" );
		}
		else
		{
				...
				var e = Date.now();
      //console.log( "Time to pick (internal)" + (e-s) + " ms" );
		}
  ...
  SelectionHelper.prototype.filterIntersects = function( intersects ){
		...
		for(var i=0;i<intersects.length;i++)
		{
		  ...
		  if( validNode && finalInteresects.indexOf( validNode ) ==-1 && !validNode.userData.unselectable ) finalInteresects.push( validNode);
		}
  ...
  SelectionHelper.prototype.getObjectAt = function(x, y, isOrtho) {
    var isOrtho = isOrtho || false;
    var s = Date.now();
		var intersects = this.pick(x,y,isOrtho);
		intersects = this.filterIntersects( intersects );
    if (intersects.length > 0) {
